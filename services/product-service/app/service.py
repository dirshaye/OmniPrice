    ...
    async def ListProducts(self, request, context):
        ...
        try:
            ...
            if request.HasField("search"):
                # Text search on name and description using regex pattern
                search_pattern = re.compile(request.search, re.IGNORECASE)
                conditions.append(
                    Or(
                        Product.name == search_pattern,
                        Product.description == search_pattern
                    )
                )
